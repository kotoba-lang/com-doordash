(ns doordash.main
  "Kotodama WASM entrypoint for the Doordash clean-room actor (L4) — Clojure port.
  ...
  facts (`doordash.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "doordash")
(def tier "L4")
...
(def entity-specs
  [{:entity "Order"     :plural "orders"     :id-prefix "doordash_ord"
   ...
   {:entity "Tracking"  :plural "trackings"  :id-prefix "doordash_tra"
...
(defn emit-facts
  "EAVT facts for one record: {\"doordash.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "doordash-compat" :tier tier :entities entities} 200])
...
(defn start! [] :doordash-compat/ready)
